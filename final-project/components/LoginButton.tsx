...
export default function LoginButton() {
  ...
  const { track, reset } = useTracking();

  const clearAllUserLocalStorage = () => {
    // Clear all user-specific localStorage keys
    const keysToRemove: string[] = [];
    
    // Collect all user-specific keys
  ...
  const handleLogin = async (provider: string) => {
    track('user_login_attempt', { provider });
    
    // Clear any existing user data before logging in (prevents data leakage)
    ...
    if (provider === 'credentials') {
      ...
      if (result?.ok) {
        track('user_login_success', { provider });
        // Ensure account record exists in MongoDB for dev user
        try {
          ...
          console.error('Failed to ensure account record:', error);
        }
        window.location.reload();
      } else {
        track('user_login_failed', { provider });
      }
    } else {
      ...
      <div className="auth-user-container">
        <div className="auth-user-info">
          ...
          <div className="auth-user-details">
            <p className="auth-user-name">{session.user?.name || 'User'}</p>
            <p className="auth-user-email">{session.user?.email}</p>
          </div>
        </div>
        <button
  ...
  return (
    <div className="auth-buttons-container">
      ...
      >
        ...
        <span className="auth-button-text">Google</span>
      </button>
      <button
        onClick={() => handleLogin('github')}
        className="auth-button auth-button-github"
      >
        <svg className="auth-icon" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          <path
            fillRule="evenodd"
        ...
        <span className="auth-button-text">GitHub</span>
      </button>
      {(process.env.NODE_ENV === 'development' || process.env.NEXT_PUBLIC_ENABLE_TEST_AUTH === 'true') && (
        <button
          onClick={() => handleLogin('credentials')}
          className="auth-button auth-button-dev"
          style={{ backgroundColor: '#10b981', color: 'white' }}
        >
          <span className="auth-button-text">Dev Login</span>
        </button>
      )}
    </div>
  );
